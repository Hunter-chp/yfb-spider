...
# 设备大类关键词（CT/DSA/MR）
KEYWORDS_DSA = ['DSA', '数字减影', '血管机', '血管造影']
KEYWORDS_CT = ['CT', '计算机断层扫描', '计算机体层摄影']
KEYWORDS_MR = ['MR', '磁共振', '核磁']

# 其他关键词（用于搜索，保持原样）
KEYWORDS_OTHER = ['医用X线', '医用设备', '医疗设备', '设备更新', '医共体', '卫生健康']
...
def determine_content_type(title, detail_text=''):
    ...
    # 第一优先级：工程、服务、仪器（只返回第一个匹配到的类别）
    if any(kw in title_lower for kw in KEYWORDS_CONSTRUCTION):
        return '工程类'
    if any(kw in title_lower for kw in KEYWORDS_SERVICE):
        return '服务类'
    if any(kw in title_lower for kw in KEYWORDS_INSTRUMENT):
        return '仪器类'

    # 第二优先级：CT/DSA/MR（标题中匹配，可能多个）
    matched_title = []
    if any(kw in title_lower for kw in KEYWORDS_CT):
        matched_title.append('CT类')
    if any(kw in title_lower for kw in KEYWORDS_DSA):
        matched_title.append('DSA类')
    if any(kw in title_lower for kw in KEYWORDS_MR):
        matched_title.append('MR类')
    if matched_title:
        return ', '.join(matched_title)

    # 第三优先级：从公告详情中匹配CT/DSA/MR
    if detail_text:
        detail_lower = detail_text.lower()
        matched_detail = []
        if any(kw in detail_lower for kw in KEYWORDS_CT):
            matched_detail.append('CT类')
        if any(kw in detail_lower for kw in KEYWORDS_DSA):
            matched_detail.append('DSA类')
        if any(kw in detail_lower for kw in KEYWORDS_MR):
...
def get_cookies_from_manual_login():
    ...
    if driver is None:
        print("错误：无法启动任何支持的浏览器。请安装 Chrome、Edge 或 Firefox。")
        sys.exit(1)
